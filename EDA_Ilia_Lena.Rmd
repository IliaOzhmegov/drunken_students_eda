...
* replacing numbers with categorical values in such variables as ‘Salc’, 
  ‘Walc’, ‘Dalc’, ‘famrel’, ‘freetime’, ‘goout’, ‘health’, ‘Medu’, ‘Fedu’
...
Unfortunately, in terms of the university work we will not cover all variables
due to the ordinary lack of time. So we will only focus on some of them. We will
attempt to answer the following questions:

+ What part of the week corresponds to the highest consumption of alcohol?
+ Does alcohol affect success?
+ Is it possible to determine certain groups of students by their alcohol consumption and grades? 

+ Do age and gender influence the grades?
+ Does it make sense to apply correlation to categorical variables?

+ Who drinks more: males or females?
+ Which age group consumes more alcohol?
...
+ How does leisure (free time after school, going out with friends) effect alcohol consumption?

# Brief introduction into alcohol consumption

Before diving deeper, let us start with an overview of alcohol consumption and 
compare it depending on a weekday or a weekend.


```{r, fig.cap = "Comparison of alcohol consumption during weekend and weekday"}
    ...
    labs(y = "Number of students", 
         ...
         title = "Comparison of alcohol consumption during weekend and weekday") +
    theme(plot.title = element_text(hjust = 0.5))
```
Besides a clear conclusion that 
...
Under the success we mean high grades at least for now.
When referring to success, we mean high grades, at least for now.
...
```{r, fig.cap = "Final Grade Correlation"}
...
Result: in general there is a poor dependence between variables and final grade,
but it is important to remember that all variables are categorical, except for 
`failures` and `absences` (`failures` are not quite continuous). Also, 
...
numeric marks from 0 to 20, we needed to transform them into  the five-level 
...
```{r, fig.cap = "Final Grade Distribution"}
{  ddf <- full_df %>% 
    mutate(
...
```
As can be seen the amount of students in both groups is almost the same.


Let us have a look directly at dependence between Grades and Alcohol consumption.

```{r, fig.cap = "Final Grade (Discrete) Distribution"}
{
...
```{r, fig.cap = "Final Grade (Discrete) Mosaic"}
  class_df %>% 
    mutate(
...
## Dependence between Grades and variables such as gender and age

How does age and gender affect students' success?
...
```{r, fig.cap = "Weekday alcohol consumption per Age and Gender"}
{
...
```{r, fig.cap = "Weekend alcohol consumption per Age and Gender"}
{
...
```{r, fig.cap = "Weekday alcohol consumption depending on the Area"}
{
...
```{r, fig.cap = "Alcohol consumption per students' family size"}
{
...
```{r, fig.cap = "Alcohol consumption depending on students' free time and go out time"}
{
...
```{r, fig.cap = "Grades by gender and weekday alcohol consumption"}
{
...
```{r, fig.cap = "Grades by gender and weekend alcohol consumption"}
{
...
1. P. Cortez and A. Silva. Using Data Mining to Predict Secondary School Student 
   ...
   EUROSIS, ISBN 978-9077381-39-7. [link](https://www.researchgate.net/profile/Paulo_Cortez3/publication/228780408_Using_data_mining_to_predict_secondary_school_student_performance/links/0fcfd510a4efaa10b0000000/Using-data-mining-to-predict-secondary-school-student-performance.pdf)

2. [Student Performance Data Set](https://archive.ics.uci.edu/ml/datasets/student+performance#)
